...
module fifo_design (
  ...
  assign data_out = mem[rd_ptr]; // 0-cycle latency
...
// ============================================================================
// 6. MONITOR  -- UNCHANGED (still samples on negedge)
// ============================================================================
...
class fifo_scoreboard extends uvm_scoreboard;
  ...
  // Set when a read drains the last word; consumed on the *next*
  // transaction, since empty is a registered signal (via NBA on count)
  // and only becomes visible one clock after the draining read.
  bit check_empty_pending;
  ...
  function void write(fifo_seq_item item);
    ...
    // ---- Deferred empty check from the previous transaction --------------
    // Checked first, against THIS item, i.e. one cycle after the read
    // that drained ref_queue -- not the same cycle as the drain.
    if (check_empty_pending) begin
      check_empty_pending = 0;
      if (item.empty)
        `uvm_info("SCB", "PASS: FIFO empty correctly asserted", UVM_LOW)
      else
        `uvm_error("SCB", "FIFO should report empty one cycle after drain, but empty=0")
    end
    ...
    if (item.rd_en && !item.empty) begin
      if (ref_queue.size() == 0) begin
        `uvm_error("SCB", "Read asserted but reference queue is empty")
      end else begin
        bit [7:0] exp_data = ref_queue.pop_front();
        if (item.data_out == exp_data)
          `uvm_info("SCB", "PASS: Data Match", UVM_LOW)
        else
          `uvm_error("SCB", $sformatf("FAIL: Expected %0h, Got %0h", exp_data, item.data_out))
      end

      // Queue just went empty -- schedule the check for the NEXT
      // transaction rather than checking item.empty right now.
      if (ref_queue.size() == 0)
        check_empty_pending = 1;
    end

    // ---- Full protection check (unchanged) --------------------------
    if (item.full) begin
...
module tb_top;
  ...
  initial begin
    // Provide virtual interface to all components (driver, monitor, scoreboard)
